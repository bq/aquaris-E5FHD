 ...
 * configuration
*******************************************************************************/
#define PSCTRL_VAL	0x33 //0x71	/* ps_persistance=4, ps_gain=64X, PS_IT=0.391ms */
#define ALSCTRL_VAL	0x3c //0x38	/* als_persistance=1, als_gain=64X, ALS_IT=50ms */
#define LEDCTRL_VAL	0xFF	/* 100mA IRDR, 64/64 LED duty */
#define WAIT_VAL 0x5	//0x7		/* 50 ms */
...
volatile static int val_temp = 1;
...
static ssize_t stk3x1x_store_config(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{ 
		...
		stk3x1x_obj->hw->ps_threshold_high= hthres;
		stk3x1x_obj->hw->ps_threshold_low = lthres;
...
static ssize_t stk3x1x_store_als(struct device_driver *ddri, const char *buf, size_t count)
{
	int enable_als = 0,err;
	u8 dat;
	if(!stk3x1x_obj)
	{
		APS_ERR("stk3x1x_obj is null!!\n");
	...
	else if(1 != sscanf(buf, "%d", &enable_als))
	{
		APS_ERR("invalid format: '%d'\n", buf);
		return 0;
	}
	if(1 == enable_als)
	{
		if((err = stk3x1x_enable_als(stk3x1x_obj->client, 1)))
		{
	...
	{
		if((err = stk3x1x_enable_als(stk3x1x_obj->client, 0)))
		{
			APS_ERR("enable als fail: %ld\n", err);
			return count;
		}
...
static ssize_t stk3x1x_store_ps(struct device_driver *ddri, const char *buf, size_t count)
{
	int enable_ps = 0,err;
	u8 dat;
	if(!stk3x1x_obj)
	...
	else if(1 != sscanf(buf, "%d", &enable_ps))
	{
		APS_ERR("invalid format: '%d'\n", buf);
		return 0;
	}
	if(1 == enable_ps)
	{
		if((err = stk3x1x_enable_ps(stk3x1x_obj->client, 1)))
		{
	...
	{
		if((err = stk3x1x_enable_ps(stk3x1x_obj->client, 0)))
		{
			APS_ERR("enable ps fail: %ld\n", err);
			return count;
		}
...
static ssize_t stk3x1x_show_product_id(struct device_driver *ddri, char *buf)
{
	int res;
	if(!stk3x1x_obj)
	{
...
static ssize_t stk3x1x_show_pscalibrate(struct device_driver *ddri, char *buf)
{
	int32_t word_data;
	u8 r_buf[2];
	int ret,i = 0;
	...
	{
		if(ret = stk3x1x_read_ps(stk3x1x_obj->client,&data))
		{
	...
	data = (int)(sum / Calibrate_num) + STK_HT_N_CT;
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	stk3x1x_ReadChipInfo(client, strbuf, 256);
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);        
}
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(chipinfo,   S_IWUSR | S_IRUGO, show_chipinfo_value,      NULL);
static DRIVER_ATTR(als,     S_IWUSR | S_IRUGO, stk3x1x_show_als,   stk3x1x_store_als);
static DRIVER_ATTR(ps,      S_IWUSR | S_IRUGO, stk3x1x_show_ps,    stk3x1x_store_ps);
...
static DRIVER_ATTR(pscalibrate,  S_IWUSR | S_IRUGO, stk3x1x_show_pscalibrate,  NULL);
static DRIVER_ATTR(id,      S_IWUSR | S_IRUGO, stk3x1x_show_product_id,   NULL);
/*----------------------------------------------------------------------------*/
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_PS_CALI:
            for(i=0; i<Calibrate_num; i++)
            {
                if(ret = stk3x1x_read_ps(obj->client,&data))
                {
                    err = -EFAULT;
                    goto err_out;
                }
            ...
            data = (unsigned int)(sum / Calibrate_num) + STK_HT_N_CT;
		...
		case ALSPS_SET_PS_CALI:
			...
			if(dat <= (STK_HT_N_CT - STK_LT_N_CT))
				dat = ps_threshold_high;
			atomic_set(&obj->ps_high_thd_val, dat);
			atomic_set(&obj->ps_low_thd_val, (dat - (STK_HT_N_CT - STK_LT_N_CT)));
			obj->hw->ps_threshold_high = dat;
			obj->hw->ps_threshold_low= dat - (STK_HT_N_CT - STK_LT_N_CT);
			if((err = stk3x1x_write_ps_high_thd(client, atomic_read(&obj->ps_high_thd_val))))
			{
				APS_ERR("write high thd error: %d\n", err);
				return err;
