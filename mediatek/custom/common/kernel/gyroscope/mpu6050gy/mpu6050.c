...
extern struct mutex mpu6050_power; // 共用加速度的 苏 勇 2013年12月09日 16:55:42
/*----------------------------------------------------------------------------*/
...
static int MPU6050_SetPowerMode(struct i2c_client *client, bool enable)
{
...
#ifdef MPU6050_ACCESS_BY_GSE_I2C
    if (MPU6050_hwmsen_read_byte(MPU6050_REG_PWR_CTL2, databuf2))
#else
    if (hwmsen_read_byte(client, MPU6050_REG_PWR_CTL2, databuf2))
#endif 
    {
    ...
    databuf2[0] = MPU6050_REG_PWR_CTL2;

#ifdef MPU6050_ACCESS_BY_GSE_I2C
    res = MPU6050_i2c_master_send(databuf2, 0x2);
#else
...
#ifdef MPU6050_ACCESS_BY_GSE_I2C
    obj->hw->addr = 0xD2;//MPU6050_I2C_SLAVE_ADDR; /* mtk i2c not allow to probe two same address */
#endif
...
static int  mpu6050_set_gyro_direction(struct i2c_client *client,int direction)
{
    int err = -1;
    struct mpu6050_i2c_data *obj = i2c_get_clientdata(client);
			...
			char *buf, loff_t off, size_t count)
{
    ...
    {
        mpu6050_set_gyro_direction(mpu6050_i2c_client,gsensor_i2c_data->hw->direction);
