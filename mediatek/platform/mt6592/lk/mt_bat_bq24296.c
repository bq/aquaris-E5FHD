...
void bq24296_set_ac_current(void)
{
    int ichg_val;
        printf("[BATTERY:bq24296] bq24296_set_ac_current \r\n");
    ...
    {
        printf("[BATTERY:bq24296] invalid current selected (%d), use 500mA \r\n", g_temp_CC_value);
        bq24296_set_ichg(0); //offset is 500mA
    }
    else
    {
#if defined(MTK_JEITA_STANDARD_SUPPORT)        
        ...
        {    
            ichg_val = 0; //lowest possible charge current for bq24296 is 500mA
        }
        else
        {
        ...
        ichg_val = (g_temp_CC_value - 500)/64;
#endif    
        bq24296_set_ichg(ichg_val);
    }
}

void bq24296_set_low_chg_current(void)
{
    if (Enable_BATDRV_LOG == 1) {
        printf("[BATTERY:bq24296] bq24296_set_low_chg_current \r\n");
    }
    bq24296_set_iinlim(0x0); //IN current limit at 100mA	
    bq24296_set_ichg(0); //charging current limit at 500mA
}

void select_charging_curret_bq24296(void)
{

            printf("Power/Battery", "[BATTERY:bq24296] bq24296_set_ac_current(), CC value(%d) \r\n", BMT_status.charger_type );
    ...
    {
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24296] Unknown charger type\n");
        }
        g_temp_CC_value = 500;
        bq24296_set_iinlim(0x2); //IN current limit at 500mA		  
        bq24296_set_ac_current();     		
...
void ChargerHwInit_bq24296(void)
{
	if (Enable_BATDRV_LOG == 1) {
		printf("[BATTERY:bq24296] ChargerHwInit_bq24296\n" );
	}
...
void pchr_turn_off_charging_bq24296 (void)
{
	if (Enable_BATDRV_LOG == 1) {
		printf("[BATTERY:bq24296] pchr_turn_off_charging_bq24296!\r\n");
	}
	...
	{
	    printf("[BATTERY:bq24296] over voltage, enable high impedance mode!\r\n");
        bq24296_set_en_hiz(0x1);
    }
}

void pchr_turn_on_charging_bq24296 (void)
{
	if ( BMT_status.bat_charging_state == CHR_ERROR )
	{
		if (Enable_BATDRV_LOG == 1) {
			printf("[BATTERY:bq24296] Charger Error, turn OFF charging !\r\n");
		}
		BMT_status.total_charging_time = 0;
		pchr_turn_off_charging_bq24296();
	}
	else
	{
 		ChargerHwInit_bq24296();

		if (Enable_BATDRV_LOG == 1) {
			printf("[BATTERY:bq24296] pchr_turn_on_charging_bq24296 !\r\n");
		}

		select_charging_curret_bq24296();
    ...
    if (low_bat_boot_display == 0)
    {
        ...
        if(display == KAL_TRUE)
        {
            ...
            if (bl_switch_timer > BL_SWITCH_TIMEOUT) {
                ...
                mt_disp_power(FALSE);
                g_bl_on = 0;
                printf("[BATTERY:bq24296] mt65xx_backlight_off\r\n");
				return;
            }
        ...
        else
        {
            mt65xx_backlight_off();
            mt_disp_power(FALSE);
            printf("[BATTERY:bq24296] mt65xx_backlight_off due to (display == KAL_FALSE)\r\n");
        }
    }
    else
    {
...
int BAT_CheckBatteryStatus_bq24296(void)
{
    long tmo;
#ifndef MTK_BQ27541_SUPPORT
    static int cnt = 9;
    ...
    if(bat_temperature_volt == 0)
    {
        ...
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24296] Warning !! bat_temperature_volt == 0, restore temperature value\n\r");
        }
    }
    else
    {
    ...
    if (Enable_BATDRV_LOG == 1) {
        printf("[BATTERY:ADC:bq24296] VCHR:%d BAT_SENSE:%d I_SENSE:%d TBAT:%d TBAT_vol:%d\n",
...
//		bq24296_set_batfet_disable(0x0);		
        ...
        printf("[BATTERY:bq24296] vol_batfet_disabled = %d\n", vol_batfet_disabled);
	}
#else
    ...
    //if (Enable_BATDRV_LOG == 1) {
    printf("[BATTERY:AVG:bq24296(%d,%dmA)] BatTemp:%d Vbat:%d VBatSen:%d SOC:%d ChrDet:%d Vchrin:%d Icharging:%d(%d) ChrType:%d \r\n",
...
#if defined(MTK_JEITA_STANDARD_SUPPORT)
    if (Enable_BATDRV_LOG == 1) {
        printf("[BATTERY:bq24296] support JEITA, Tbat=%d\n", BMT_status.temperature);			 
    }
    ...
    {
        printf(  "[BATTERY:bq24296] Battery Under Temperature or NTC fail !!\n\r"); 			   
        BMT_status.bat_charging_state = CHR_ERROR;
        return PMU_STATUS_FAIL; 	  
    }
#endif                
    ...
    {
        printf(  "[BATTERY:bq24296] Battery Over Temperature !!\n\r");				  
        BMT_status.bat_charging_state = CHR_ERROR;
        return PMU_STATUS_FAIL; 	  
    }
#endif    
    ...
    if( upmu_is_chr_det() == KAL_TRUE)
    {
        ...
        {
            printf(  "[BATTERY:bq24296]Charger under voltage!!\r\n");
            BMT_status.bat_charging_state = CHR_ERROR;
            return PMU_STATUS_FAIL;
        }
        #endif
        ...
        {
            printf(  "[BATTERY:bq24296]Charger over voltage !!\r\n");
    ...
    if( upmu_is_chr_det() == KAL_TRUE)
    {
        ...
        {
            if (Enable_BATDRV_LOG == 1) {
            	printf("[BATTERY:bq24296] Battery Re-charging !!\n\r");
            }
...
{
    if (Enable_BATDRV_LOG == 1) {
    	printf("[BATTERY:bq24296] BAD Battery status... Charging Stop !!\n\r");
    }
...
{
    printf("[BATTERY:bq24296] Charging over %d hr stop !!\n\r", MAX_CHARGING_TIME);
...
{
    if (Enable_BATDRV_LOG == 1) {    
        printf("[BATTERY:bq24296] check Battery full !!\n\r");            
    }
...
//// bq24296
///////////////////////////////////////////////////////////////////////////////////////////
void pmic_init_for_bq24296(void)
{
	printf("[pmic_init_for_bq24296] Start\n");
	...
	printf("[pmic_init_for_bq24296] Done\n");
}
...
void BAT_thread_bq24296(void)
{
    long tmo;
    int BAT_status = 0;
    kal_uint32 bq24296_stat=0;
    int i = 0;
    int chr_err_cnt = 0;

    printf("[BAT_thread_bq24296] mtk_wdt_restart()\n");
    mtk_wdt_restart();

    if (Enable_BATDRV_LOG == 1) {
        printf("[BAT_thread_bq24296] LOG. %d,%d----------------------------\n", BATTERY_AVERAGE_SIZE, RECHARGING_VOLTAGE);
    }    
...
//    if(get_charger_hv_status() == 1 || (upmu_is_chr_det() == KAL_FALSE))
    if(upmu_is_chr_det() == KAL_FALSE)
    {
        ...
        printf("[BATTERY:bq24296] No Charger, Power OFF !?\n");
        pchr_turn_off_charging_bq24296();

        printf("[BATTERY:bq24296] mt_power_off !!\n");
	#ifndef NO_POWER_OFF
        mt6575_power_off();
	#endif
        while(1);
    ...
    BAT_status = BAT_CheckBatteryStatus_bq24296();
    ...
    if(BAT_status == PMU_STATUS_FAIL || g_Battery_Fail)
    {
        ...
        if (Enable_BATDRV_LOG == 1)
        {
            printf("[BATTERY:bq24296] PMU_STATUS_FAIL (chr_err_cnt = %d)\n", chr_err_cnt);
        }
    }
    /* Battery Full */
    ...
    else if (g_HW_Charging_Done == 1)
    {
        if (Enable_BATDRV_LOG == 1)
        {
            printf("[BATTERY:bq24296] Battery real full. \n");
        }
        BMT_status.bat_full = KAL_TRUE;
        BMT_status.total_charging_time = 0;
        BMT_status.PRE_charging_time = 0;
    ...
    else if (g_Charging_Over_Time == 1)
    {
        if (Enable_BATDRV_LOG == 1)
        {
            printf("[BATTERY:bq24296] Charging Over Time. \n");
        }
        pchr_turn_off_charging_bq24296();
    }
    ...
    else
    {
        if (Enable_BATDRV_LOG == 1)
        {
            printf("[BATTERY:bq24296] state=%d, \n", BMT_status.bat_charging_state);
        }
        ...
        bq24296_stat = bq24296_get_chrg_stat();

        /* check battery full */
#if defined(MTK_JEITA_STANDARD_SUPPORT)
        if((BMT_status.bat_vol > g_jeita_recharging_voltage) && (bq24296_stat == 0x3))
#else
        if((BMT_status.bat_vol > RECHARGING_VOLTAGE) && (bq24296_stat == 0x3))
#endif
        {
            ...
            g_HW_Charging_Done = 1;
            pchr_turn_off_charging_bq24296();
            printf("[BATTERY:bq24296] Battery real full and disable charging (%d) \n", bq24296_stat);
            return;
        }
        else if(bq24296_stat == 0x3) //false alarm, reset status
        {
            pchr_turn_off_charging_bq24296();
            tmo = get_timer(0);
            while(get_timer(tmo) <= 100 /* ms */);
            if (Enable_BATDRV_LOG == 1) {
                printf("[BATTERY:bq24296] bq24296_status is (%d), but battery voltage not over recharge voltage\n", bq24296_stat);
            }
        }
        ...
        BMT_status.bat_charging_state = CHR_CC;
        pchr_turn_on_charging_bq24296();

        if (Enable_BATDRV_LOG == 1)
        {
            printf(  "[BATTERY:bq24296] Total charging timer=%d, bq24296_stat=%d \n\r",
            BMT_status.total_charging_time, bq24296_stat);
        }
    }
...
void batdrv_init(void)
{
    ...
    if ((upmu_is_chr_det() == KAL_TRUE))
    {
        printf("bq24296 hw init\n");
        bq24296_dump_register();
        pmic_init_for_bq24296();
        pchr_turn_on_charging_bq24296();
    }
    ...
    printf("[BATTERY:bq24296] batdrv_init : Done\n");
}
...
void mt65xx_bat_init(void)
{
...
#endif		
    {
        ...
        pchr_turn_off_charging_bq24296(); //turn off charging at first to prevent initial voltage measure reach 4.2 when pushing reset button and adaptor plugin
        BMT_status.bat_vol = get_i_sense_volt_ch0(5);
    }
    ...
    if ((upmu_is_chr_det() == KAL_TRUE))
    {
        ...
        pchr_turn_on_charging_bq24296(); //turn on charging for powerpath
