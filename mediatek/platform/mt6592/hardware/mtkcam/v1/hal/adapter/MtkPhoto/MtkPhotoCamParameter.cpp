...
#define UPDATE_PARAMS(param, eMapXXX, key) \
    ...
    //add by wjh
    if(::strcmp((mpParamsMgr->getStr("anr-enable")).string(), "true") == 0){
        MY_LOGI("VEGETA-1215: ANR set to Enable");
        cam3aParam. bIsANREnabled = MTRUE;
    }else{
        MY_LOGI("VEGETA-1215: ANR set to Disable");
        cam3aParam. bIsANREnabled = MFALSE;
	}
//add by wjh end
