...
u32 pinSet[2][8] = {
   ...
   else
   {
    ...
    if (On) {
		...
		#if 1//main-dvdd conncet to VGP3 and ov12830 use externel LDO
        ...
        if(pinSetIdx == 0 || pinSetIdx == 2) {//disable sub
		   ...
		   ||(currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8865QT_MIPI_RAW,currSensorName))))
		{
	...
	{
		PK_DBG("[pengliu]kdCISModulePowerOn get in---  SENSOR_DRVNAME_OV8865_MIPI_RAW \n");
		PK_DBG("kdCISModulePowerOn get in--- %s \n",currSensorName);
    ...
    else
    {
		...
		{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_OV12830_MIPI_RAW or SENSOR_DRVNAME_OV12830_MIPI_RAW or SENSOR_DRVNAME_OV5648_MIPI_RAW \n");
		...
		{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_GC2355_MIPI_RAW \n");
            PK_DBG("kdCISModulePower--off get in---%s\n",currSensorName);
            //reset pull down
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
				if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
