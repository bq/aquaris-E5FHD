...
static void hall_switch_handler(unsigned long data);
static DECLARE_TASKLET(hall_switch_tasklet, hall_switch_handler, 0);
static u8 hall_switch_state = CUST_EINT_MHALL_TYPE;
#endif
...
static BOOL has_otherkey_pressed = FALSE;
static BOOL pwrkey_rst_enabled = TRUE; // already enabled in function long_press_reboot_function_setting in kpd_hal.c
#endif
static void kpd_aee_handler(u32 keycode, u16 pressed)
{
	if (pressed) {
		...
		#ifdef ONEKEY_REBOOT_NORMAL_MODE
		has_otherkey_pressed = TRUE;
		pwrkey_rst_enabled = FALSE;
		upmu_set_rg_pwrkey_rst_en(0x0);
		#endif
	} else {
		if (keycode == KEY_VOLUMEUP) {
...
{
    bool slid;
    u8 old_state = hall_switch_state;
    u8 cur_hall_light_level = mt_get_bl_brightness();

    mt_eint_mask(CUST_EINT_MHALL_NUM);

    hall_switch_state = !hall_switch_state;

    slid = (hall_switch_state == !!CUST_EINT_MHALL_TYPE);
    printk("hall report hall = %s\n", slid ? "far away" : "closed");
    printk("hall cur_hall_light_level = %d, hall_switch_state = %d\n", cur_hall_light_level, hall_switch_state);
    ...
    else if((1==slid)&&(0 == cur_hall_light_level)&&(0==g_is_calling))
    {
        input_report_key(kpd_input_dev, KPD_PWRKEY_MAP, 1);
        input_sync(kpd_input_dev);
        udelay(10);
...
void kpd_pwrkey_pmic_handler(unsigned long pressed)
{
	...
	#ifdef ONEKEY_REBOOT_NORMAL_MODE
	if((pwrkey_rst_enabled == FALSE) && (has_otherkey_pressed == FALSE) && pressed)
	{
		pwrkey_rst_enabled = TRUE;
...
static int kpd_open(struct input_dev *dev)
{
    ...
    mt_eint_registration(CUST_EINT_MHALL_NUM, CUST_EINT_MHALL_TYPE,hall_switch_eint_handler, false);
    mt_eint_unmask(CUST_EINT_MHALL_NUM);
#endif
...
#if CKT_HALL_SWITCH_SUPPORT
	err = hall_create_attr(&kpd_pdrv.driver);
#endif
