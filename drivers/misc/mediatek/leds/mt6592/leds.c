...
{
	printk("kolery mt_mt65xx_led_set_cust\n");
	...
	*/
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_PWM:
			LEDS_DEBUG("kolery MT65XX_LED_MODE_PWM\n");
		...
		case MT65XX_LED_MODE_GPIO:
			LEDS_DEBUG("kolery MT65XX_LED_MODE_GPIO\n");
			LEDS_DEBUG("brightness_set_cust:go GPIO mode!!!!!\n");
			return ((cust_set_brightness)(cust->data))(level);
              
		case MT65XX_LED_MODE_PMIC:
			LEDS_DEBUG("kolery MT65XX_LED_MODE_PMIC\n");
		...
		case MT65XX_LED_MODE_CUST_LCM:
			LEDS_DEBUG("kolery MT65XX_LED_MODE_CUST_LCM\n");
            if(strcmp(cust->name,"lcd-backlight") == 0)
			{
			    bl_brightness_hal = level;
            }
		...
		case MT65XX_LED_MODE_CUST_BLS_PWM:
			LEDS_DEBUG("kolery MT65XX_LED_MODE_CUST_BLS_PWM\n");
			if(strcmp(cust->name,"lcd-backlight") == 0)
			{
				bl_brightness_hal = level;
			}
...
#ifdef CONFIG_BULMA_PROJECT
printk("kolery CONFIG_BULMA_PROJECT\n");
        case MT65XX_LED_MODE_CUST:
		LEDS_DEBUG("kolery MT65XX_LED_MODE_CUST\n");	
            ...
            LEDS_DEBUG("==== mt_mt65xx_led_set_cust:led=%d level = %d====\n",led,level);
            if(level == 0) {
                Suspend_led();
            }
            else {
...
int mt_led_blink_pmic_cust(struct nled_setting* led) {
	...
	for(i = 0; i < 3 ;  i++ )
	{
		...
		if(rgb[i] != 0)
		{
		printk("[phil] curr %s LED,data %d\n",p_cust_led_list[i].name,p_cust_led_list[i].data);
