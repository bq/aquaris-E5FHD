...
static int __init ram_console_early_init(void)
{
    ...
    if (bufp_dram->sig == REBOOT_REASON_SIG) {
        pr_err("ram_console: kernel use sram, but pl/lk use dram.\n");
	memset(bufp, 0x0, buffer_size);
        memcpy(bufp, bufp_dram, sizeof(struct ram_console_buffer));
	memcpy((void *)bufp + bufp->off_pl, (void *)bufp_dram + bufp->off_pl, sizeof(bufp->sz_pl));
	pr_err("ram_console: dram wdt_status:%x, sram wdt_status:%x\n", LAST_RRPL_BUF_VAL(bufp_dram, wdt_status), LAST_RRPL_BUF_VAL(bufp, wdt_status));
        bufp_dram->sig = 0;
    }
#endif
