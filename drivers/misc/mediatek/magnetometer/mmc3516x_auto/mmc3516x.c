...
static int MMCECS_CheckDevice(void)
{
	char buffer[2];
	int ret;
//	printk(" mmc3516x check device id\n");
	/* Set measure mode */
	buffer[0] = MMC3516X_REG_PRODUCTID_1;

	/* Read data */
	ret = I2C_RxData(buffer, 1);
	printk(" mmc3516x check device id = %x\n",buffer[0]);
//	printk(" mmc3516x ret = %d\n",ret);
	if(ret < 0)
	{
		return ret;
	}
...
//static int mmc3516x_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
	static long mmc3516x_unlocked_ioctl( struct file *file, unsigned int cmd,unsigned long arg)
...
#ifdef CONFIG_COMPAT
static long mmc3516x_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = mmc3516x_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
...
static int mmc3516x_m_enable(int en)
{
	int value = 0;
	int err = 0;
	value = en;
...
static int mmc3516x_m_set_delay(u64 ns)
{
...
static int mmc3516x_m_open_report_data(int open)
{
	return 0;
}

static int mmc3516x_m_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
static int mmc3516x_o_enable(int en)
{
	int value = 0;
	int err = 0;
	value = en;
...
static int mmc3516x_o_set_delay(u64 ns)
{
	int value = 0;
	value = (int)ns/1000/1000;
	if(value <= 10)
...
static int mmc3516x_o_open_report_data(int open)
{
	return 0;
}

static int mmc3516x_o_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
//modified for mmc3516x----changhua start
	...
	if(err < 0)
	{
		MAG_ERR("mmc3516x probe: check device connect error\n");
...
*/
	...
	ctl.m_enable = mmc3516x_m_enable;
	ctl.m_set_delay  = mmc3516x_m_set_delay;
	ctl.m_open_report_data = mmc3516x_m_open_report_data;
	ctl.o_enable = mmc3516x_o_enable;
	ctl.o_set_delay  = mmc3516x_o_set_delay;
	ctl.o_open_report_data = mmc3516x_o_open_report_data;
...
*/
	...
	mag_data.get_data_o = mmc3516x_o_get_data;
	mag_data.get_data_m = mmc3516x_m_get_data;
...
static int mmc3516x_i2c_remove(struct i2c_client *client)
{
	...
	*/
	if((err = mmc3516x_delete_attr(&(mmc3516x_init_info.platform_diver_addr->driver))))
	{
		printk(KERN_ERR "mmc3516x_delete_attr fail: %d\n", err);
	}
...
static int	mmc3516x_local_init(void)
{
	...
	{
		MMCDBG("mmc3516x add driver error\n");
		return -1;
	}
...
static int __init mmc3516x_init(void)
{
	...
	i2c_register_board_info(hw->i2c_num, &i2c_mmc3516x, 1);
