...
int RG_Ratio_Typical = 0x148;
int BG_Ratio_Typical = 0x132;
...
static int check_otp_wb(int index)
{
				...
				// read flag
				address = 0x3d00;
				flag = OV8850_read_cmos_sensor(address);
				flag = flag & 0xc0;
				SENSORDB("check_otp_wb, flag = %x\n", flag);
				// clear otp buffer
				for (i=0;i<16;i++) {
							OV8850_write_cmos_sensor(0x3d00 + i, 0x00);
				}
...
static int read_otp_wb(int index, struct otp_struct * otp_ptr)
{
				...
				mdelay(5);
				address = 0x3d00;
				(*otp_ptr).module_integrator_id = OV8850_read_cmos_sensor(address + 1);
				(*otp_ptr).lens_id = OV8850_read_cmos_sensor(address + 2);
				(*otp_ptr).production_year = OV8850_read_cmos_sensor(address + 3);
				(*otp_ptr).production_month = OV8850_read_cmos_sensor(address + 4);
				(*otp_ptr).production_day = OV8850_read_cmos_sensor(address + 5);
				temp = OV8850_read_cmos_sensor(address + 10);
				(*otp_ptr).rg_ratio = (OV8850_read_cmos_sensor(address + 6)<<2) + ((temp>>6) & 0x03);
				(*otp_ptr).bg_ratio = (OV8850_read_cmos_sensor(address + 7)<<2) + ((temp>>4) & 0x03);
				(*otp_ptr).light_rg = (OV8850_read_cmos_sensor(address + 8) <<2) + ((temp>>2) & 0x03);
				(*otp_ptr).light_bg = (OV8850_read_cmos_sensor(address + 9)<<2) + (temp & 0x03);
				//(*otp_ptr).user_data[0] = OV8850_read_cmos_sensor(address + 11);
				//(*otp_ptr).user_data[1] = OV8850_read_cmos_sensor(address + 12);
				//(*otp_ptr).user_data[2] = OV8850_read_cmos_sensor(address + 13);
				//(*otp_ptr).user_data[3] = OV8850_read_cmos_sensor(address + 14);
				//(*otp_ptr).user_data[4] = OV8850_read_cmos_sensor(address + 15);
			SENSORDB("read_otp_wb, module_integrator_id = %x\n", (*otp_ptr).module_integrator_id);
			SENSORDB("read_otp_wb, rg_ratio = %x\n", (*otp_ptr).rg_ratio);
			SENSORDB("read_otp_wb, bg_ratio = %x\n", (*otp_ptr).bg_ratio);
			SENSORDB("read_otp_wb, light_rg = %x\n", (*otp_ptr).light_rg);
			SENSORDB("read_otp_wb, light_bg = %x\n", (*otp_ptr).light_bg);
...
static int update_blc_ratio()
{
					...
					OV8850_write_cmos_sensor(0x3d81, 0x01);
					mdelay(5);
					K = OV8850_read_cmos_sensor(0x3d0b);
					if (K != 0) {
									if (K >= 0x15 && K <= 0x40) {
													// auto load mode
													temp = OV8850_read_cmos_sensor(0x4008);
													temp &= 0xfb;
													OV8850_write_cmos_sensor(0x4008, temp);
							...
							K = OV8850_read_cmos_sensor(0x3d0a);
					if (K >= 0x10 && K <= 0x40) {
										// manual load mode
										OV8850_write_cmos_sensor(0x4006, K);
										temp = OV8850_read_cmos_sensor(0x4008);
										temp &= 0xfb;
										OV8850_write_cmos_sensor(0x4008, temp);
										temp = OV8850_read_cmos_sensor(0x4000);
					...
					temp |= 0x08;
					OV8850_write_cmos_sensor(0x4000, temp);
		SENSORDB("update_blc_ratio, 0x4000 = %x\n", OV8850_read_cmos_sensor(0x4000));
    SENSORDB("update_blc_ratio, 0x4006 = %x\n", OV8850_read_cmos_sensor(0x4006));
					return 0;
					}
}
