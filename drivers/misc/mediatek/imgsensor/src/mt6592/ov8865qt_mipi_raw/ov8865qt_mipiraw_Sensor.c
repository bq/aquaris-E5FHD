...
#include "ov8865qt_mipiraw_Sensor.h"
#include "ov8865qt_mipiraw_Camera_Sensor_para.h"
#include "ov8865qt_mipiraw_CameraCustomized.h"
static DEFINE_SPINLOCK(ov8865mipiraw_drv_lock);

#define OV8865_DEBUG
#define LOG_TAG "[OV8865qtRaw]"
#ifdef OV8865_DEBUG
	#define OV8865DB(fmt, arg...) printk(LOG_TAG "%s: " fmt "\n", __FUNCTION__ ,##arg)
#else
	#define OV8865DB(fmt, arg...)
#endif


static kal_uint32 OV8865_FeatureControl_PERIOD_PixelNum=OV8865_PV_PERIOD_PIXEL_NUMS;
static kal_uint32 OV8865_FeatureControl_PERIOD_LineNum=OV8865_PV_PERIOD_LINE_NUMS;

static UINT16 OV8865_VIDEO_MODE_TARGET_FPS = 30;

static MSDK_SCENARIO_ID_ENUM OV8865CurrentScenarioId = MSDK_SCENARIO_ID_CAMERA_PREVIEW;
static MSDK_SENSOR_CONFIG_STRUCT OV8865SensorConfigData;
static OV8865_PARA_STRUCT ov8865;
static kal_uint32 OV8865_FAC_SENSOR_REG;


static SENSOR_REG_STRUCT OV8865SensorCCT[]=CAMERA_SENSOR_CCT_DEFAULT_VALUE;
static SENSOR_REG_STRUCT OV8865SensorReg[ENGINEER_END]=CAMERA_SENSOR_REG_DEFAULT_VALUE;


#define OV8865_TEST_PATTERN_CHECKSUM 0xf5e2f1ce
static kal_bool OV8865_During_testpattern = KAL_FALSE;

extern int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId);
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);

#define OV8865_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, OV8865QT_MIPI_WRITE_ID)

static kal_uint16 OV8865_read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte,OV8865QT_MIPI_WRITE_ID);
    return get_byte;
}
...
static int RG_Ratio_Typical = 0x126;//0x230;
static int BG_Ratio_Typical = 0x122;// 0x2B1;
...
static void OV8865_Init_Para(void)
{
...
static kal_uint32 GetOv8865LineLength(void)
{
...
static kal_uint32 GetOv8865FrameLength(void)
{
...
static kal_uint32 OV8865_CalcExtra_For_ShutterMargin(kal_uint32 shutter_value,kal_uint32 shutterLimitation)
{
...
static kal_uint32 OV8865_CalcFrameLength_For_AutoFlicker(void)
{
...
static void OV8865_write_shutter(kal_uint32 shutter)
{
	...
	OV8865DB("ov8825qt write shutter=%x, line_length=%x, frame_length=%x\n", shutter, line_length, frame_length);
...
static void write_OV8865_gain(kal_uint16 gain)
{
...
static void OV8865_SetGain(UINT16 iGain)
{
	unsigned long flags;
	spin_lock_irqsave(&ov8865mipiraw_drv_lock,flags);

	OV8865DB("OV8865qt_SetGain iGain = %d :\n ",iGain);
	...
	OV8865DB(" [OV8865qt_SetGain]ov8865.sensorGlobalGain=0x%x,ov8865.realGain =%x",ov8865.sensorGlobalGain,
		ov8865.realGain); 
...
static kal_uint16 read_OV8865_gain(void)
{
	kal_uint16 read_gain=0;
	...
	OV8865DB("ov8865qt.sensorGlobalGain=0x%x,ov8865.realGain=%d\n",ov8865.sensorGlobalGain,ov8865.realGain);
...
#if 1
static void OV8865_camera_para_to_sensor(void)
{
    kal_uint32    i;
...
static void OV8865_sensor_to_camera_para(void)
{
...
static kal_int32  OV8865_get_sensor_group_count(void)
{
    return GROUP_TOTAL_NUMS;
}

static void OV8865_get_sensor_group_info(kal_uint16 group_idx, kal_int8* group_name_ptr, kal_int32* item_count_ptr)
{
   switch (group_idx)
   {
        case PRE_GAIN:
...
static void OV8865_get_sensor_item_info(kal_uint16 group_idx,kal_uint16 item_idx, MSDK_SENSOR_ITEM_INFO_STRUCT* info_ptr)
{
...
static kal_bool OV8865_set_sensor_item_info(kal_uint16 group_idx, kal_uint16 item_idx, kal_int32 ItemValue)
{
   kal_uint16  temp_gain=0,temp_addr=0, temp_para=0;
...
static void OV8865PreviewSetting(void)
{
...
static void OV8865VideoSetting(void)
{
...
static void OV8865CaptureSetting(void)
{
...
static UINT32 OV8865qtOpen(void)
{

	volatile signed int i;
	kal_uint16 sensor_id = 0;

	OV8865DB("OV8865qt Open enter :\n ");
	...
	for(i=0;i<2;i++)
	{
		...
		sensor_id += 1;
		OV8865DB("OV8865qt READ ID :%x",sensor_id);
		if(sensor_id != OV8865QT_SENSOR_ID)
		{
...
#endif	
	OV8865_write_cmos_sensor( 0x5002, 0x00);//bit[3]=0 OTP_DPC function disable
	update_otp_info();
	mdelay(2);
	update_otp_wb();
	mdelay(2);
	update_otp_lenc();
	OV8865_write_cmos_sensor( 0x5002, 0x08);//bit[3]=1 OTP_DPC function enable
	mdelay(2);
	#endif

	
	OV8865DB("OV8865qtOpen exit :\n ");

    return ERROR_NONE;
}


static UINT32 OV8865qtGetSensorID(UINT32 *sensorID)
{
    int  retry = 2;

	OV8865DB("OV8865qtGetSensorID enter :\n ");
    mdelay(5);

    do {
        *sensorID = (OV8865_read_cmos_sensor(0x300B)<<8)|OV8865_read_cmos_sensor(0x300C);
		*sensorID = *sensorID + 1;
        if (*sensorID == OV8865QT_SENSOR_ID)
        	{
...
static void OV8865_SetShutter(kal_uint32 iShutter)
{
...
static UINT32 OV8865_read_shutter(void)
{
...
static void OV8865_NightMode(kal_bool bEnable)
{

}

static UINT32 OV8865qtClose(void)
{
...
static UINT32 OV8865Preview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{

	OV8865DB("OV8865qtPreview enter:");
...
static UINT32 OV8865Video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{

	OV8865DB("OV8865qtVideo enter:");
    ...
    mdelay(40);//THIS DELAY SHOULD BE NEED BY CTS OR MONKEY
	OV8865DB("OV8865qtVideo exit:\n");
    return ERROR_NONE;
}


static UINT32 OV8865Capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	{
		OV8865DB("OV8865qtCapture BusrtShot / ZSD!!!\n");
	}
	else
	{
		OV8865DB("OV8865qtCapture enter:\n");
...
static UINT32 OV8865qtGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{

    OV8865DB("OV8865qtGetResolution!!\n");
...
static UINT32 OV8865qtGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId,
...
static UINT32 OV8865qtControl(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
		spin_lock(&ov8865mipiraw_drv_lock);
		...
		OV8865DB("hkm_0815 OV8865qtCurrentScenarioId=%d\n",OV8865CurrentScenarioId);
...
static kal_uint32 OV8865_SET_FrameLength_ByVideoMode(UINT16 Video_TargetFps)
{
    UINT32 frameRate = 0;
...
static UINT32 OV8865qtSetVideoMode(UINT16 u2FrameRate)
{

    kal_uint32 MIN_Frame_length =0,frameRate=0,extralines=0;
    OV8865DB("[OV8865qtSetVideoMode] frame rate = %d\n", u2FrameRate);
	...
	OV8865DB("[OV8865qtSetVideoMode]MIN_Frame_length=%d,ov8865.DummyLines=%d\n",MIN_Frame_length,ov8865.DummyLines);

    return KAL_TRUE;
}


static UINT32 OV8865qtSetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
	...
	if(bEnable) {   
		...
		spin_unlock(&ov8865mipiraw_drv_lock);
        OV8865DB("OV8865qt Enable Auto flicker\n");
    } else {
    	spin_lock(&ov8865mipiraw_drv_lock);
        ov8865.OV8865AutoFlickerMode = KAL_FALSE;
		spin_unlock(&ov8865mipiraw_drv_lock);
        OV8865DB("OV8865qt Disable Auto flicker\n");
    }
...
static UINT32 OV8865qtSetTestPatternMode(kal_bool bEnable)
{
    OV8865DB("[OV8865qtSetTestPatternMode] Test pattern enable:%d\n", bEnable);
    if(bEnable == KAL_TRUE)
    {
...
* 
*************************************************************************/
static UINT32 OV8865qtMIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	kal_uint32 pclk;
	kal_int16 dummyLine;
	kal_uint16 lineLength,frameHeight;
		
	OV8865DB("OV8865qtMIPISetMaxFramerateByScenario: scenarioId = %d, frame rate = %d\n",scenarioId,frameRate);
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
...
static UINT32 OV8865qtMIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
...
static UINT32 OV8865qtFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId,
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
            if (*pFeatureParaLen>=sizeof(NVRAM_SENSOR_DATA_STRUCT))
            {
                ...
                pSensorDefaultData->SensorId=OV8865QT_SENSOR_ID;
        ...
        case SENSOR_FEATURE_SET_VIDEO_MODE:
            OV8865qtSetVideoMode(*pFeatureData16);
            break;
        case SENSOR_FEATURE_CHECK_SENSOR_ID:
            OV8865qtGetSensorID(pFeatureReturnPara32);
            break;
        case SENSOR_FEATURE_SET_AUTO_FLICKER_MODE:
            OV8865qtSetAutoFlickerMode((BOOL)*pFeatureData16, *(pFeatureData16+1));
	        break;
		case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
			OV8865qtMIPISetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*pFeatureData32, *(pFeatureData32+1));
			break;
		case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
			OV8865qtMIPIGetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*pFeatureData32, (MUINT32 *)(*(pFeatureData32+1)));
			break;
		case SENSOR_FEATURE_SET_TEST_PATTERN:
			OV8865qtSetTestPatternMode((BOOL)*pFeatureData16);
			break;
...
SENSOR_FUNCTION_STRUCT	SensorFuncOV8865QT=
{
    OV8865qtOpen,
    OV8865qtGetInfo,
    OV8865qtGetResolution,
    OV8865qtFeatureControl,
    OV8865qtControl,
    OV8865qtClose
};
