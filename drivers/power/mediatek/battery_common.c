...
//20140929 xiexijun add ckt baseline AT cmd for factory test begin
static ssize_t show_Ckt_AT_Test_Charging_State(struct device *dev,struct device_attribute *attr, char *buf)
{
    battery_xlog_printk(BAT_LOG_CRTI, "call state = %d\n",g_test_state);    
    return sprintf(buf, "%u\n", g_test_state);
}
static ssize_t store_Ckt_AT_Test_Charging_State(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
       sscanf(buf, "%u", &g_test_state);
    battery_xlog_printk(BAT_LOG_CRTI, "call state = %d\n",g_test_state);    
    return size;
}
static DEVICE_ATTR(Ckt_AT_Test_Charging_State, 0664, show_Ckt_AT_Test_Charging_State, store_Ckt_AT_Test_Charging_State);

static ssize_t show_ckt_batt_voltage(struct device *dev,struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n",  battery_meter_get_battery_voltage(KAL_TRUE));
}
static ssize_t store_ckt_batt_voltage(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return 0;
}
static DEVICE_ATTR(ckt_batt_voltage, 0664, show_ckt_batt_voltage, store_ckt_batt_voltage);



kal_bool g_test_state = KAL_FALSE;
static PMU_STATUS mt_battery_CheckTestState(void)
{
PMU_STATUS status = PMU_STATUS_OK;
if(g_test_state == KAL_TRUE)
status = PMU_STATUS_FAIL;

return status;
}
//20140929 xiexijun add ckt baseline AT cmd for factory test end
...
#endif
//20140929 xiexijun add ckt baseline AT cmd for factory test begin
       if(mt_battery_CheckTestState() != PMU_STATUS_OK)
       {
    battery_xlog_printk(BAT_LOG_CRTI, "mt_battery_CheckBatteryStatus = %d\n",g_test_state);  
               BMT_status.bat_charging_state = CHR_HOLD;
        return;                  
  }
//20140929 xiexijun add ckt baseline AT cmd for factory test end
...
#ifdef TEMPERATURE_CKT_WARNING 
	if (BMT_status.temperature >= MAX_HIGH_NOTIFY_TEMPERATURE) /*ckt guoyi add CKT  temperature NOTIFY 2015-6-26 */
#else
	if (BMT_status.temperature >= batt_cust_data.max_charge_temperature)
#endif
	{
...
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
#ifdef TEMPERATURE_CKT_WARNING
    else if (BMT_status.temperature <= MIN_LOW_NOTIFY_TEMPERATURE) /*ckt guoyi add CKT  temperature NOTIFY 2015-6-26 */
#else
...
#else
#ifdef TEMPERATURE_CKT_WARNING
		if (BMT_status.temperature >= MAX_CHARGE_POWEROFF_TEMPERATURE) /*ckt guoyi add CKT  temperature NOTIFY 2015-6-26 */
#else
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Pump_Express);
           ...
           //20140929 xiexijun add ckt baseline AT cmd for factory test begin
           ret_device_file = device_create_file(&(dev->dev), &dev_attr_Ckt_AT_Test_Charging_State);
           
           ret_device_file = device_create_file(&(dev->dev), &dev_attr_ckt_batt_voltage);
      //20140929 xiexijun add ckt baseline AT cmd for factory test end
	}
