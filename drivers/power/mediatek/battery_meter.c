...
static ssize_t show_FG_HW_version(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    if(900 <= hw_ver_adc && hw_ver_adc < 1450) 
    {
        return sprintf(buf, "%s\n", "PCB Ver: 3.0");
    }
    else if(600 <= hw_ver_adc && hw_ver_adc < 900)  
    {
        return sprintf(buf, "%s\n", "PCB Ver: 2.0");
    }
    else
...
static ssize_t store_FG_HW_version(struct device *dev, struct device_attribute *attr, const char *buf,
				size_t size)
{
	return size;
}

static DEVICE_ATTR(FG_HW_version, 0664, show_FG_HW_version, store_FG_HW_version);
